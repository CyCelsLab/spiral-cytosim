...
minl = tempdf.length.min()
maxl = tempdf.length.max()

fig,ax = plt.subplots(figsize=(4,3))
plt.scatter((tempdf.lfd),(tempdf.A),edgecolor=cmap((tempdf.length-minl)/(maxl-minl)),facecolor="none")
...
#ax.tick_params(axis='both', which='major', label1On=False)

ax.tick_params(axis='both', which='minor', labelsize=15, label1On=True)
ax.xaxis.set_minor_locator(FixedLocator([5,10,15,20,25]))
...
plt.tight_layout()
#plt.show()
plt.savefig(f"{figureFolder}/scaled-vl-A-f.pdf")
...
plt.tight_layout()
#plt.show()
plt.savefig(f"{figureFolder}/scaled-cl-A-f.pdf")
