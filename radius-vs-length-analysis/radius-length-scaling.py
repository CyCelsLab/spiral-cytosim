...
#%%%% rescaling data
dfvl["xi"] =  dfvl["radius"]*dfvl["lfd"]**(1/3)
#%%%% plotting rescaled data
tempdf = dfvl
tempdf = tempdf.groupby(["length","density"]).mean().reset_index()

fig,ax = plt.subplots(figsize=(4,3))
plt.scatter(tempdf.length,tempdf.xi,c=tempdf.density,cmap="turbo")
plt.ylabel(r"$\xi$ $(\mu m^{2} pN)^{1/3}$",fontsize=15)
plt.xlabel(r"$\ell_{MT}$ $(\mu m)$",fontsize=15)
plt.tick_params(labelsize=15)
...
x = np.sort(x.unique())
plt.plot(x,a*x**b,ls="--",label=rf"$\alpha$:{round(b,3)}",lw=3,color="k")
plt.xticks(ticks=[7, 14, 21, 28])
plt.ylim(1,6)

plt.text(16,4,r"$\ell_{MT}^{%s}$"%round(b,3),fontsize=15)
plt.tight_layout()
#plt.show()
plt.savefig(f"{figureFolder}/scaled-vl-xi-length.pdf")
...
#%%%% rescaling data
dfcl["xi"] =  dfcl["radius"]*dfcl["lfd"]**(1/3)
#%%%% plotting rescaled data
tempdf = dfcl
tempdf = tempdf.groupby(["length","density"]).mean().reset_index()
...
plt.xticks(ticks=[7, 14, 21, 28])
plt.ylim(1,6)
plt.text(16,3.5,r"$\ell_{MT}^{%s}$"%round(b,3),fontsize=15)
plt.tight_layout()
#plt.show()
plt.savefig(f"{figureFolder}/scaled-cl-xi-length.pdf")
#%%%% plotting rescaled data with overlaying theoretical fit for variable persistence length
tempdf = dfvl
tempdf = tempdf.groupby(["length","density"]).mean().reset_index()
...
plt.scatter(tempdf.length,tempdf.xi,facecolor="none",edgecolor=cmap((tempdf.density-dmin)/(dmax-dmin)))
...
x = tempdf.length
y = tempdf.xi
fit1 = lambda x,a: a*Kbt*pl(x)**(1/3)
(a),(_) = curve_fit(fit1,x,y)
x = np.linspace(1,50,200)
plt.plot(x,fit1(x,a),ls="-.",label=rf"{np.round(a[0],3)}$KT\ell_{{p}}({{\ell}})^{{1/3}}$",lw=2,color="navy")
